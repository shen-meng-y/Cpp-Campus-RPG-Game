...
int Quest::getConditionValue() const {
...
int Quest::getProgressValue(const Character& player, int killCount) const {
    int value = 0;
    ...
    return std::max(0, std::min(value, conditionValue));
}
...
std::vector<Quest> Quest::createDefaultQuests() {
    return {
        Quest("清理校园", "击败 3 个敌人。", QuestType::KillEnemy, 3, 60, 50),
        Quest("努力成长", "角色等级达到 2 级。", QuestType::ReachLevel, 2, 60, 30),
        Quest("金币积累", "金币数量达到 150。", QuestType::GoldAmount, 150, 30, 40),
        Quest("校园勇者", "累计击败 10 个敌人。", QuestType::KillEnemy, 10, 120, 100),
        Quest("进阶成长", "角色等级达到 3 级。", QuestType::ReachLevel, 3, 100, 80),
        Quest("小富校园", "金币数量达到 300。", QuestType::GoldAmount, 300, 80, 120)
    };
}
